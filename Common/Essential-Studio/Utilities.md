...
2. The Syncfusion Build Manager x.x.x.x window opens.
   ...
   N> Build Manager is available in the Dashboard only when Source code is installed. You can also launch the Build Manager from {ProgramFilesFolder}\Syncfusion\Essential Studio\x.x.x.x\Utilities\Build Manager\Buildmanagerwindows.exe
